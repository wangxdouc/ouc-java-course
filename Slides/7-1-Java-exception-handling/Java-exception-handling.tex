...
\begin{frame}[fragile]
  ...
  \notice{提供异常处理机制的的基本思想}

  让一个函数在发现了自己无法处理的错误时抛出（throw）一个异常，然后它的
  （直接或者间接）调用者能够处理这个问题。
  
  %% The fundamental idea is that a function that finds a problem it
  %% cannot cope with throws an exception, hoping that its (direct or
  %% indirect) caller can handle the problem.
